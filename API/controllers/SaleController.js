...
// Endpoint to save a sale transaction
...
// Get details for a specific bill sale
...
// Update the sale status to 'sent'
...
// Get dashboard summary for sales per month
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            for (let j = 0; j < billSaleInMonth.length; j++) {
                ...
                const sum = await prisma.billSaleDetail.aggregate({
                    _sum: { price: true },
                    where: { billSaleId: billSaleObject.id }
                });
